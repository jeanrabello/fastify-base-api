...
describe("FindUserByIdUseCase", () => {
  ...
  describe("Successful execution", () => {
    it("Should return partial user data when user is found", async () => {
      const mockUser: Partial<User> = {
        ...
        password: existingUser.password, // This should not be returned
      ...
      // Should only return id, username, and email (without password and timestamps)
      ...
      // Verify password and other fields are not included
  ...
  describe("Input validation", () => {
    ...
    it("Should process whitespace-only id (truthy but likely invalid)", async () => {
      // Note: Whitespace string is truthy in JavaScript, so it passes the !id check
      // This test verifies the current behavior - it will call the repository
      userRepository.findById.mockResolvedValue(null);
  ...
  describe("Data filtering", () => {
    it("Should only return id, username, and email fields", async () => {
      ...
      // Verify sensitive and metadata fields are filtered out
